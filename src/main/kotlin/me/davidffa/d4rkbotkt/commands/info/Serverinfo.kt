...
class Serverinfo : Command(
    ...
    override suspend fun run(ctx: CommandContext) {
        var online = 0; var idle = 0; var dnd = 0; var offline = 0
        ...
        val embed = EmbedBuilder {
            ...
            field {
                name = ":date: Criado em"
                value = "<t:${ctx.guild.timeCreated.toInstant().epochSecond}:d> (<t:${ctx.guild.timeCreated.toInstant().epochSecond}:R>)"
            }
            field {
                name = ":calendar: Entrei em"
                value = "<t:${ctx.guild.selfMember.timeJoined.toInstant().epochSecond}:d> (<t:${ctx.guild.selfMember.timeJoined.toInstant().epochSecond}:R>)"
            }
            field {
